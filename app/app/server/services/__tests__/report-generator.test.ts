...
function it(name: string, fn: () => void) {
  try {
    fn();
    results.push({ name: `${currentGroup} > ${name}`, passed: true });
  } catch (e: any) {
    results.push({ name: `${currentGroup} > ${name}`, passed: false, error: e.message });
  }
}
...
function expect(val: any) {
  return {
    ...
    toContain(str: string) {
      if (typeof val === 'string' && !val.includes(str)) throw new Error(`Expected string to contain "${str}"`);
...
console.log(`  SOP Part 7 — Report Generator Regression Tests`);
console.log('═'.repeat(80));
