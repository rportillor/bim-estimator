...
{ "mep": [ { "mep_id","system","type","x","y","size","notes" }, ... ] }`,
  ...
  private parseLayerResponse(
    ...
    if (layer === 'gridlines') {
      ...
      // The static constants renderer (moorings-grid-constants.ts) is the sole source
      // of gridlines in the 3D viewer. Storing them here causes duplicates and stale data.
      const count = (result?.grid_lines ?? []).length;
      logger.info(`[extractLayer] SKIPPING ${count} grid_line elements — rendered from constants, not DB`);
