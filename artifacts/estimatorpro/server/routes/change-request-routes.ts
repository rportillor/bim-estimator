...
estimatedCostImpact to null and note the gap in analysisNotes. Return ONLY valid JSON.`;
      ...
      const response = await anthropic.messages.create({
        ...
        max_tokens: 1024,
        temperature: 0,
        system: "You are a Canadian quantity surveyor (MCIQS) performing change request impact analysis. Return ONLY valid JSON.",
      ...
      let analysis: any;
      try {
        analysis = parseFirstJsonObject(responseText);
        if (!analysis || analysis.error) throw new Error('No JSON object in Claude response');
      } catch (parseErr) {
