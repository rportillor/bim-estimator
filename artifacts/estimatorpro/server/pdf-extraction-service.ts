...
export async function extractPdf(filePath: string, previewDir: string): Promise<ExtractedPdf> {
  ...
  // PNG previews for first up to 8 pages
  ...
  const rasterPreviews: { page: number; filePath: string }[] = [];
  for (let p = 1; p <= limit; p++) {
    const res = await toImage(p);
    rasterPreviews.push({ page: p, filePath: res.path || '' });
  }
...
export async function updateDocumentWithExtractedContent(
  ...
  try {
    ...
    await storage.updateDocument(documentId, {
      ...
      analysisStatus: "Ready" // Mark as ready for Claude analysis
    });
    
    console.log(`✅ Document ${documentId} updated with extracted PDF content`);
    
  } catch (error) {
    console.error(`❌ Failed to update document ${documentId} with extracted content:`, error);
    throw error;
  }
}
...
export function getConstructionFocusedContent(extractedContent: ExtractedPdf, tokenLimit: number = 8000): {
  ...
  // Keywords that indicate important construction content
  ...
  // Score pages by construction content relevance
  ...
  for (const scoredPage of scoredPages) {
    const pageTokens = scoredPage.pageData.text.length / 4; // Rough token estimation
    ...
    if (salientPages.length >= 6) break; // Max 6 pages
  }
